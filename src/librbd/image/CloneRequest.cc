...
void CloneRequest<I>::send_validate_child() {
  ...
  using klass = CloneRequest<I>;
  librados::AioCompletion *comp = create_rados_ack_callback<klass, &klass::handle_validate_child>(this);
...
void CloneRequest<I>::send_create() {
  ...
  std::string id = librbd::util::generate_image_id(m_ioctx);
  m_create_req = CreateRequest<I>::create(
    m_ioctx, m_name, id, m_size, m_opts, m_non_primary_global_image_id,
...
void CloneRequest<I>::send_set_parent() {
  ...
  librados::AioCompletion *comp =
    create_rados_ack_callback<klass, &klass::handle_set_parent>(this);
...
void CloneRequest<I>::send_add_child() {
  ...
  librados::ObjectWriteOperation op;
  cls_client::add_child(&op, m_pspec, m_imctx->id);

  using klass = CloneRequest<I>;
  librados::AioCompletion *comp =
    create_rados_ack_callback<klass, &klass::handle_add_child>(this);
...
void CloneRequest<I>::send_metadata_list() {
  ...
  using klass = CloneRequest<I>;
  librados::AioCompletion *comp =
    create_rados_ack_callback<klass, &klass::handle_metadata_list>(this);
  m_out_bl.clear();
...
void CloneRequest<I>::send_metadata_set() {
  ...
  using klass = CloneRequest<I>;
  librados::AioCompletion *comp =
    create_rados_ack_callback<klass, &klass::handle_metadata_set>(this);
...
void CloneRequest<I>::send_close() {
  ...
  Context *ctx = create_async_context_callback(
    *m_imctx, create_context_callback<
      klass, &klass::handle_close, false>(this));
...
template <typename I>
void CloneRequest<I>::switch_thread_context() {
...
template <typename I>
void CloneRequest<I>::handle_switch_thread_context(int r) {
...
void CloneRequest<I>::send_remove_child() {
  ...
  librados::ObjectWriteOperation op;
  cls_client::remove_child(&op, m_pspec, m_imctx->id);
...
void CloneRequest<I>::send_remove() {
  ...
  librbd::image::RemoveRequest<> *req = librbd::image::RemoveRequest<>::create(
   m_ioctx, m_name, m_imctx->id, false, m_no_op, m_op_work_queue, ctx);
  req->send();
}

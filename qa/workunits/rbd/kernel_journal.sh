...
test_replay_from_crash()
{
    ...
    save_commit_position ${journal}
    #rbd image-meta set ${image} conf_rbd_qos_iops_limit 10
    expect_false timeout 3 rbd bench-write ${image} --io-size 4096 --io-threads 1 \
        --io-pattern rand
    restore_commit_position ${journal}

    rbd journal export ${journal} $TMPDIR/journal.export
    rbd snap add ${image}@snap
    rbd export ${image}@snap $TMPDIR/${image}.export
